...
export type RobotExpression = 'curious' | 'thinking' | 'happy' | 'worried' | 'sad' | 'seeing' | 'listening' | 'playful' | 'laughing';
...
export function getRobotExpression(input: RobotRuntimeState | RobotExpressionContext): RobotExpression {
  ...
  if (jokePattern.test(signal)) return 'laughing';
...
export function getRobotExpressionHeadline(expression: RobotExpression) {
  ...
  if (expression === 'laughing') return '哈哈～';
  return '嗨，我在這裡';
}
